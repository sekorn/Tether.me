...
class Syncer: NSObject {
    ...
    func backgroundContextSaved(notification: NSNotification) {
        mainContext.performBlock({
            // the below line of code forces coredata to load the full entity object so we can see it was updated.  
            self.objectsForKey(NSUpdatedObjectsKey, dictionary: notification.userInfo!, context: self.mainContext).forEach{
                $0.willAccessValueForKey(nil)
            }
